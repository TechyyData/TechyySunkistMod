...
import techyy.sunkistmod.SunkistMod;

import java.util.*;

/**
 * Created by fabbe on 23/12/2017 - 8:29 AM.
 */
        ...
        public static void registerBlocks(final RegistryEvent.Register<Block> event) {
            ...
            for (final Block block : blocks) {
                if (block.getRegistryName() != null) {
                    ...
                    SunkistMod.getLogger().info("Successfully added block: " + block.getRegistryName().getResourcePath() + " to the game.");
                } else
                    SunkistMod.getLogger().warn("Tried to register block without registry name, ignoring.");
            }
        }
        ...
        public static void registerItems(RegistryEvent.Register<Item> event) {
            ...
            for (Item item : items) {
                if (item.getRegistryName() != null) {
                    reg.register(item);
                    ITEM_LIST.add(item);
                    SunkistMod.getLogger().info("Successfully added item: " + item.getRegistryName().getResourcePath() + " to the game.");
                } else
                    SunkistMod.getLogger().warn("Tried to register item without registry name, ignoring.");
            }
        }
